...
public class TopCometStreamImpl implements TopCometStream {
	...
	public void start() {
		...
		for(TopCometStreamRequest cometRequest : cometRequests){
			...
			new Thread(ct,"stream-control-thread-connectid-" + cometRequest.getConnectId()).start();
		}
	}
	...
	public class ControlThread implements Runnable {
		...
		private StreamImplementation getMsgStreamImpl()	throws TopCometSysErrorException ,Exception{
			...
			if (!StringUtils.isEmpty(cometReq.getConnectId())){
				param.put(StreamConstants.PARAM_CONNECT_ID, cometReq.getConnectId());
			}
			...
			String sign = null;
			try {
				sign = TaobaoUtils.signTopRequestNew(paramHolder, cometReq.getSecret(),
						false);
			...
			return new MessageStreamImpl(msgConsumeFactory,	response, cometReq.getMsgListener(), this);
		}
				...
				while (!gloableStop && !closed && stream.isAlive()) {
					try {
						...
						if (System.currentTimeMillis()
								- ct.lastStartConsumeThread < 30 * 60 * 1000) {// 短时间内由于读取IOException连接了10次，则退出
	...
	public void stop() {
		...
		gloableStop = true;
		/*
		 * 先等待最大时间，最大时间是一个心跳包的间隔时间
		 * 停止接收数据的线程
		 */
//		try{
//			//等待一点时间，让阻塞读数据的线程从阻塞读返回来。这里休眠的时间稍微比心跳包的间隔大一点
